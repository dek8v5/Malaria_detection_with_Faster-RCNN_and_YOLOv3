...
def predict_single_image(img_path, model_rpn, model_classifier_only, cfg, class_mapping):
    ...
    for cls_num, box in boxes.items():
        ...
        for b in boxes_nms:
            ...
            if class_mapping[cls_num] == 'rbc':
               rbc_not+=1
            if class_mapping[cls_num] == 'infected':
               infected+=1
    ...
    print("total rbc in this image: " + str(rbc_not) + " infected: " + str(infected))
    cv2.imwrite(result_path, img)
    cv2.waitKey(0)
    return rbc_not, infected
...
def predict(args_):
	...
	#cfg = pickle.dump(f_in, protocol=2)
	#cfg = pickle.load(f_in)
    ...
    base_net_weights = './model/malaria_model_17800.hdf5'
    ...
    with open(path, 'r') as filehandler:
        ...
        for img_name in lines:
            ...
            print("total infected all img: " + str(total_infected))
    '''
    cfg.simple_label_file = 'test_malaria_frcnn.txt'
    all_images, classes_count, class_mapping = get_data(cfg.simple_label_file)

    print(all_images['filepath'])
...
def parse_args():
    ...
    parser.add_argument('--path', '-p', default='/storage/hpc/group/kazic-lab/NN/frcnn/test_malaria.txt', help='image path')
